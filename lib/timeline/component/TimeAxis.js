...
function TimeAxis (body, options) {
  ...
  this.defaultOptions = {
    ...
    minorLabelCharMeasurer: '0',
    majorLabelCharMeasurer: '0',
    maxMinorChars: 7,
...
TimeAxis.prototype.setOptions = function(options) {
  if (options) {
    ...
    util.selectiveExtend([
      'showMinorLabels',
      'showMajorLabels',
      'minorLabelCharMeasurer',
      'majorLabelCharMeasurer',
      'maxMinorChars',
      'hiddenDates',
      'timeAxis',
      'moment',
...
TimeAxis.prototype._repaintLabels = function () {
  ...
  while (step.hasNext() && count < MAX) {
    ...
    if (isMajor && this.options.showMajorLabels) {
      ...
      this._repaintSubscale(step, current, next, x, orientation, className);
      ...
      // no new line -> adjust the width of the *previous* line
...
TimeAxis.prototype._calculateCharSize = function () {
  ...
  if (!this.dom.measureCharMinor) {
    ...
    this.dom.measureCharMinor.appendChild(document.createElement('br'));
    this.dom.measureCharMinor.appendChild(document.createTextNode('0'));
...
TimeAxis.prototype._repaintSubscale = function (step, current, next, x, orientation, className) {
  var stepDuration = next - current;

  var minimumSubscaleWidth = 8.0; // FIXME-sebu: config
  ...
  if (!subscale) {
    // console.log('subscale: could not derive subscale from step', step)
    return;
  }
  ...
  switch (subscale) {
    case 'year':
      ...
      // FIXME-sebu: logic redundant with TimeStep.setMinimumStep(). can we use that code here? Or even use a TimeStep instance for substeps?
      // we can safely assume that step.scale is 'year'
      switch (step.step) {
        case 1000:
          substepDuration = 500;
          break;
  ...
  while (subscaleDate.valueOf() < next.valueOf()) {
    var subscaleX = this.body.util.toScreen(subscaleDate);

    // Checks minimum width on first iteration
    if (index === 0 && (subscaleX - x < minimumSubscaleWidth)) {
      /*
      console.log('subscale too small', {
        subscaleX,
        x,
        minimumSubscaleWidth,
        subscaleWidth: subscaleX - x
      })
      //*/
      break;
    }
 ...
 * @private
 */
TimeAxis.prototype._repaintSubstepLine = function (x, width, orientation, className) {
  // reuse redundant line
  var line = this.dom.redundant.lines.shift();
  if (!line) {
    // create vertical line
  ...
  var props = this.props;
  /*
  console.log('_repaintSubstepLine', {
    orientation, className,
    'props.majorLabelHeight': props.majorLabelHeight,
    'props.minorLabelHeight': props.minorLabelHeight,
  ...
  if (this.options.rtl) {
    ...
    line.className = 'vis-grid vis-vertical-rtl vis-substep ' + className;
  } else {
    line.style.left = (x - width/2) + 'px';
    line.className = 'vis-grid vis-vertical vis-substep ' + className;
  }
  line.style.width = width + 'px'; // explicitly set width because of line reuse
  /*
  console.log('_repaintSubstepLine result', {
    top: line.style.top,    
